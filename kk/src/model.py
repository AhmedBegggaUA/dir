...
class GNN(torch.nn.Module):
    def __init__(
        ...
        if num_layers == 1:
            self.convs = ModuleList([get_conv(conv_type, num_features+hidden_dim, output_dim, self.alpha)])
        else:
            self.convs = ModuleList([get_conv(conv_type, num_features+hidden_dim, hidden_dim, self.alpha)])
